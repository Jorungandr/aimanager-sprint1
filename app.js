...
let current = null, roles = [], users = [], projects = [];
...
async function refresh() {
  ...
  $('currentUser').textContent = current.email; $('adminPanel').hidden = !current.is_admin;
  roles = await api('/api/roles'); projects = await api('/api/projects'); users = current.is_admin ? await api('/api/users') : [];
  $('roleCount').textContent = roles.length; $('projectCount').textContent = projects.length;
  ...
  for (const project of projects) {
    ...
    api(`/api/projects/${project.id}/members`).then(members => {
      ...
      for (const member of members) {
        ...
        if (current.is_admin) { const remove = document.createElement('button'); remove.className = 'danger'; remove.textContent = '移出项目'; remove.addEventListener('click', async () => { if (!confirm(`移出 ${member.email}？`)) return; try { await api(`/api/projects/${project.id}/members/${member.user_id}`, { method: 'DELETE' }); say('成员已移出，权限已回收'); await refresh(); } catch (e) { say(e.message, true); } }); row.append(remove); }
        list.append(row);
      }
    ...
    if (current.is_admin) {
      ...
      const roleSelect = document.createElement('select'); roleSelect.name = 'role_id'; roleSelect.setAttribute('aria-label', '选择角色'); roles.forEach(r => roleSelect.append(option(r.id, r.name)));
